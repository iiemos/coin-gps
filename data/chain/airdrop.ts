export default {
  data: [
    {
      Name: 'Coinmarketcap',
      Icon: '/img/exchange/CoinMarketCap.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://coinmarketcap.com/zh/airdrop/',
      Desc: `参与专属加密货币空投活动`,
    },
    {
      Name: 'Airdrop Alert',
      Icon: '/img/airdrop/AirdropAlert.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://airdropalert.com/',
      Desc: `赚取加密货币并加入最佳空投、赠品等！`,
    },
    {
      Name: 'DropsEarn',
      Icon: '/img/airdrop/DropsEarn.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://dropsearn.com/airdrops/',
      Desc: `用 ❤ 为去中心化世界打造。`,
    },
    {
      Name: 'Cosmos airdrops',
      Icon: '/img/airdrop/CosmosAirdrops.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://www.cosmosairdrops.io/',
      Desc: `在 Cosmos 生态系统中寻找最热门的新空投。`,
    },
    {
      Name: 'Airdrops',
      Icon: '',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://airdrops.io/',
      Desc: `真实、有价值的加密货币空投列表。`,
    },
    {
      Name: 'notion',
      Icon: '/img/airdrop/Notion.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://zlexdl.notion.site/db2e69b82a364d9e8bd356c4c73a228c?v=b3a4e992cd014603975d50df4e0fe9af',
      Desc: `空投交互教程`,
    },
    {
      Name: 'Notion',
      Icon: '/img/airdrop/Notion.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://mysterious-helicopter-d3e.notion.site/c82f9630a2a54d4384a27db97591d093?v=0e8f5d3b955547a8a30c346bf3f0cffc',
      Desc: `最新空投交互教程，包括Aptos，L2的各项目交互教程`,
    },
    {
      Name: 'Earni',
      Icon: '/img/airdrop/Earni.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://earni.fi/',
      Desc: `获取新空投通知，可查看任意钱包地址的空投。`,
    },
    {
      Name: 'MESSARI',
      Icon: '/img/airdrop/MESSARI.svg',
      Type: ['Airdrop News'],
      OfficialUrl: 'https://messari.io/fundraising-data',
      Desc: `跟踪加密风险投资、私募股权和并购交易`,
    },
  ]
}